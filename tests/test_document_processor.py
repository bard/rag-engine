...
def test_document_processor_db_interactions(sample_html_data_url, mock_db):
    # Create document processor instance
    processor = DocumentProcessor(mock_db)

    # Load data using the processor
    documents = processor.load_data(sample_html_data_url)
...
def test_document_processor_return_value(sample_html_data_url, mock_db):
    # Create document processor instance
    processor = DocumentProcessor(mock_db)
    ...
    assert all(doc.metadata["source_url"] == sample_html_data_url for doc in documents)
...
def test_document_processor_database_state(
    sample_html_data_url, sqlite_session, snapshot
):
    processor = DocumentProcessor(sqlite_session)

    processor.load_data(sample_html_data_url)
